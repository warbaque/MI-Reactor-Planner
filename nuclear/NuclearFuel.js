...
class NuclearFuel extends NuclearAbsorbable {
  constructor(
    ...
    super(
      item,
      clampTemp(maxTemperature),
      0.8 * NuclearConstant.BASE_HEAT_CONDUCTION,
      neutronBehaviour,
      desintegrationMax
    );
    ...
    this.directEUbyDesintegration = Math.floor(
      NuclearConstant.EU_FOR_FAST_NEUTRON * directEnergyFactor * neutronMultiplicationFactor
    );
    this.totalEUbyDesintegration = Math.floor(
      NuclearConstant.EU_FOR_FAST_NEUTRON * (1.0 + directEnergyFactor) * neutronMultiplicationFactor
    );
  }
...
FuelData.forEach((type) => {
  ...
  NuclearComponent.register(NuclearFuel.of(type.key, fuelParams, neutronBehaviour, type.depleted));
});
