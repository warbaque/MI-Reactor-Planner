class INeutronBehaviour {
  static of(
    ...
    const thermalProbability = INeutronBehaviour.probaFromCrossSection(
      (thermalNeutronAbsorptionBarn + thermalNeutronScatteringBarn) * Math.sqrt(size)
    );
    const fastProbability = INeutronBehaviour.probaFromCrossSection(
  ...
  static reduceCrossProba = (proba, crossSectionFactor) =>
    INeutronBehaviour.probaFromCrossSection(INeutronBehaviour.crossSectionFromProba(proba) * crossSectionFactor);

  static ofParams(scatteringType, params, size) {
    return INeutronBehaviour.of(
      scatteringType,
      params.thermalAbsorption,
      params.fastAbsorption,
      params.thermalScattering,
      params.fastScattering,
      size
    );
  }
